...
def chat(recipient_id):
    ...
    form = MessageForm()
    if form.validate_on_submit():
        message = Message(body=form.message.data, author_id=current_user.id, chat_room_id=chat_room.id)
        db.session.add(message)
        db.session.commit()
        #socketio.emit('message_received', {'message': message.body, 'author': message.author.username}, 
        #              room=chat_room.id, broadcast=True, include_self=False)
    ...
    return render_template('chat.html', recipient=recipient, messages=messages, form=form, chat_room=chat_room)
...
def handle_send_message(data):
    ...
    if room and message_body:
        emit('message_received', {'message': message_body, 'author': current_user.username}, 
